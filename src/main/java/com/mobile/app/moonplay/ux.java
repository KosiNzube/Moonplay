...
*/
        ...
        if (videouri!=null){
            ...
            reference.putFile(videouri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                ...
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText(ux.this,"failed 1"+e.getMessage(),Toast.LENGTH_SHORT).show();

                }
            });
        }else {
            Toast.makeText(ux.this,"No video selected",Toast.LENGTH_SHORT).show();
        }
    }
    private void upload(instV instant) {
        ...
        db.collection("Valbum").document(video).collection("parts").add(instant).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
                    @Override
                    public void onSuccess(DocumentReference documentReference) {
                        Toast.makeText(getApplicationContext(),"Uploaded",Toast.LENGTH_SHORT).show();


                        db.collection("Valbum").document(video).update("likes", FieldValue.increment(1));
