...
public class streamposx extends RecyclerView.Adapter<streamposx.MyViewHolder>{
    private Context context;
    private List<Streampostion> itemDataList;

    public streamposx(Context context, List<Streampostion> itemDataList) {
    ...
    public MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(context).inflate(R.layout.accountcardx,parent,false);
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, @SuppressLint("RecyclerView") final int i) {
       ...
       Picasso.get().load(itemDataList.get(i).getImages()).into(holder.img_item);
        ...
        if (itemDataList.get(i).getMio() > 3) {
            ...
            holder.genre.setText("You only started playing!");
        }
