...
public class recoplaylist extends RecyclerView.Adapter<recoplaylist.MyViewHolder> {

    private String subx;
    private Context context;
    private FirebaseFirestore db = FirebaseFirestore.getInstance();
    private CollectionReference ge=db.collection("recom");
    private List<Movie> itemDataList=new ArrayList<>();
    private final static int FADE_DURATION = 590;
    private SharedPreferences appPreferences;
    public recoplaylist(Context context) {
       ...
       // setFadeAnimation(myViewHolder.img_item);
        ...
        myViewHolder.card.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                ...
                ge.document(itemDataList.get(i).getVideo()).update("moviename", itemDataList.get(i).getName()+", " + itemDataList.get(i).getUploader()).addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void unused) {
                        Toast toast = Toast.makeText(context, itemDataList.get(i).getName()+" Added ", Toast.LENGTH_SHORT);
